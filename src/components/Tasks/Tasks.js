...
function Tasks({ tasks, colors, lists, cardTaskMenu, removTaskItem, toggleTaskItemCompleted, changeListItemName }) {

    const [inputValue, setInputValue] = useState('');

    const showAddCard = cardTaskMenu.showAddCard;
    const hideAddCard = cardTaskMenu.hideAddCard;
    const showAddCardMenu = cardTaskMenu.showAddCardMenu;
    const addTaskItem = cardTaskMenu.addTaskItem;


    const addNewTask = (listId, text, completed) => {
        if (listId && text) {
            const newListItem = {
                id: Date.now(),
                listId,
    ...
    // only one ListItem now (urlId)
    let tmpId = 2;

    let elemsToRender = lists.filter(el => el.id === tmpId).map((listItem => {
        let colorName = colors.filter(colorItem => colorItem.id === listItem.colorId)[0].name;
        return (
            <div key={listItem.id} className="tasks__card">
                <h2
                    onClick={() => {
                        const newName = prompt('Введіть нову назву', listItem.name);
                        if (newName) {
                            changeListItemName(listItem.id, newName);
                        }
                    }}
                    className={`tasks__card__name tasks__card__name--${colorName}`}>
                    {listItem.name}
                ...
                <ul>
                    {tasks.filter((e => e.listId === listItem.id)).map(taskItem => {
                        return (
                            <li key={taskItem.id}>
                                <div className="tasks__card__items__row">
                                    <div>
                                        <div className="checkbox">
                                            <input onClick={() => toggleTaskItemCompleted(taskItem.id)} id={`check${taskItem.id}`} type="checkbox" defaultChecked={taskItem.completed} />
                                            <label htmlFor={`check${taskItem.id}`}>
                                    ...
                                    <i onClick={() => removTaskItem(taskItem.id)} className="remove-task-item">
                ...
                {showAddCardMenu ? <div className="tasks__card__add">
                    <input
                        value={inputValue}
                    ...
                    <button
                        onClick={() => addNewTask(listItem.id, inputValue, false)}
                ...
                </div> : <div onClick={showAddCard} className="tasks__card__add__task">
                        <i>
