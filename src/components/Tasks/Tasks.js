...
function Tasks({ tasks, colors, lists, cardTaskMenu }) {
    ...
    let elemsToRender = lists.filter(el => el.id === tmpId).map((elem => {
        let colorName = colors.filter(el => el.id === elem.colorId)[0].name;
        return (
            <div key={elem.id} className="tasks__card">
                <h2 className={`tasks__card__name tasks__card__name--${colorName}`}>
                    {elem.name}
                ...
                <ul>
                    {tasks.filter((e => e.listId === elem.id)).map(el => {
                        return (
                            <li key={el.id}>
                                <div className="tasks__card__items__row">
                                    <div>
                                        <div className="checkbox">
                                            <input id={`check${el.id}`} type="checkbox" defaultChecked={el.completed} />
                ...
                {showAddCardMenu ? <div className="tasks__card__add">
                    ...
                    <button onClick={hideAddCard} className="tasks__card__add__addbutton closebutton" >Відмінити</button>
